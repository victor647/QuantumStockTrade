...
class SelectedPerformance(QMainWindow, Ui_SelectedPerformance):
    ...
    def batch_test_performance(self):
        ...
        total_trade_count = total_successful_trade_count = total_investment = total_profit = 0
        for date, codes in full_data.items():
            self.__startDate = QDate.fromString(date, 'yyyy-MM-dd')
            investment, profit, successful_trade_count = self.batch_trade_simulation(codes)
            total_trade_count += len(codes)
            total_investment += investment
            total_profit += profit
            total_successful_trade_count += successful_trade_count
        profit_percentage = round(total_profit / total_investment * 100, 2)
        QMessageBox.information(self, "回测完成", "共交易{}次，盈利{}次，投入{}元，盈亏{}元（{}%）".format(total_trade_count, total_successful_trade_count, total_investment, total_profit, profit_percentage))

    # 简易版批量回测
    def batch_trade_simulation(self, codes: list):
        total_investment = total_profit = successful_trade_count = 0
        for code in codes:
            data = self.get_stock_history(code)
            ...
            for day in range(1, self.spbMaxHoldTime.value()):
                ...
                # 在加仓期限之前可以进行加仓
                if day < self.spbAddDeadline.value() and bought_again is False:
                    # 达到补仓点，降低成本
        ...
        return total_investment, total_profit, successful_trade_count
